...
import { getBorderRadiusClasses, getEventColorClasses, getEventColorStyles } from "@/components/event-calendar/utils"
import type { CalendarEvent } from "@/components/event-calendar/types"
import { cn } from "@/lib/utils"

// Function to get colored circle for timeblock types
const getTimeblockCircle = (type?: string) => {
  if (!type) return null;
  ...
  return (
    <span className={`inline-block w-2 h-2 rounded-full ${colorClass} mr-1 flex-shrink-0`} />
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="text-xs opacity-70">
        {event.timeblockType ? (
          <div className="truncate capitalize">{event.timeblockType}</div>
        ) : event.location && (
